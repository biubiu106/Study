$(function(){
  ...
  $("#pdfDiv").css("height",$(window).height()-65);
  $("#right_side").css('width', $(window).width()* 93/100-350);


  $('a[left_menu_text]').mouseover(function () {
    // 鼠标放上当前目录后找到下一层目录是否为空，为空则ajax上传后台获取下层目录信息
    var obj = $(this).siblings().children().find('a[left_menu_text]');
    var att = $(this).attr('left_menu_text');
    if(obj.length>0)
    {
      $.ajax({
        url: '/ajax_menu_dir',
        ...
        success: function (ret) {
          ...
          for(var i=0; i<retJson.length; i++) {
            obj[i].innerText = retJson[i];
          }
        }
      });
    }

    $('a[func_left_menu]').mouseover(function () {
      var obj = $(this).siblings().children().find('ul');
      var att = $(this).attr('func_left_menu'); //
      if(obj.length>0)
      {
        $.ajax({
        url: '/ajax_menu_files',
  ...
  {
    $('body').removeClass();
    $('body').addClass('Hillary');
  }
  if (!$.cookie('background'))
  {
    $('body').css('background-image','url(/static/images/style_menu/patterns/3.png)');
    $.cookie('background', '3.png');
  }
  ...
  if($.cookie('color')) {
    ...
    $('body').css('background-image','url(/static/images/style_menu/patterns/' + c_background + ')' );
  }
  ...
  $('.change_background a').click(function() {
    ...
    $('body').css('background-image','url(/static/images/style_menu/patterns/' + at_background + ')' );
  ...
  // 给左侧菜单栏a标签统一添加属性,解决文字过多时挤占换行的bug，增加鼠标悬后添加title提示
  $(".left_menu").find('a').css('cursor', 'pointer').css('overflow', 'hidden').css('white-space', 'nowrap').css('text-overflow', 'ellipsis');
  $('.left_menu').find('a').mouseover(function () {
    $(this).attr('title', $(this).text());
  });


  $(".chat-tips").click(function () {
    $(this).removeClass('dis');
  });

  $(".wechat-style").mouseover(function () {
    $(".chat-tips").removeClass('dis');
  })
  $(".wechat-style").mouseleave(function () {
    $(".chat-tips").addClass('dis');
  })
});